...
public class AddCustomerController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (operation.equalsIgnoreCase("addCustomer")) {
			...
			String reEnterPwd = request.getParameter("reEnterPwd");

			if (pwd != null && reEnterPwd != null && pwd.equals(reEnterPwd)) {
				...
				request.setAttribute("reEnterPwd", "Password is not matching");
				rd = request.getRequestDispatcher("SignUp.jsp");
				rd.include(request, response);
			}
		}
			...
			if (newPwd != null && confPwd != null && newPwd.equalsIgnoreCase(confPwd)) {
				...
				if (flag > 0) {
					...
					if (updateFlag > 0) {
						...
						request.setAttribute("changePwdMsg", "Password Changed Successfully");
						rd.include(request, response);
					...
					rd = request.getRequestDispatcher("ChangePassword.jsp");
					request.setAttribute("changePwdMsg", "Your old password didn't match");
					rd.include(request, response);
				}
			} else {
				request.setAttribute("message", "Password is not matching");
				rd=request.getRequestDispatcher("ChangePassword.jsp");
				rd.include(request, response);
			}
		}
